...
_STATS_METRICS = [
    ...
    "fan_in", "fan_out", "transitive_deps", "dependency_depth",
]
...
def stats(path, lang, ignore_prefixes, config_path, show_all, top_n, show_table):
    ...
    # Graph metrics
    graph = build_dependency_graph(parsed, root)
    _collect_graph_stats(graph, values)
...
def _collect_graph_stats(graph, values):
    """Collect fan-in, fan-out, transitive deps, and dependency depth."""
    from .engine.graph import graph_fan_in, graph_fan_out, graph_indirect_dependencies
    import networkx as nx

    for node in graph.nodes:
        values["fan_in"].append(graph_fan_in(graph, node))
        values["fan_out"].append(graph_fan_out(graph, node))
        values["transitive_deps"].append(graph_indirect_dependencies(graph, node))
    for node in graph.nodes:
        try:
            depth = max(
                (nx.shortest_path_length(graph, node, t)
                 for t in nx.descendants(graph, node)),
                default=0,
            )
        except nx.NetworkXError:
            depth = 0
        values["dependency_depth"].append(depth)
...
def mimic(path, lang, ignore_prefixes, out_path):
    """Generate .kissconfig thresholds from codebase statistics (p95 clamped)."""
    content = _generate_kissconfig(path, lang, ignore_prefixes)
    if out_path:
...
def clamp(path, ignore_prefixes):
    ...
    content = _generate_kissconfig(path, None, ignore_prefixes)
    Path(".kissconfig").write_text(content, encoding="utf-8")
    print("Wrote .kissconfig")


def _generate_kissconfig(path, lang, ignore_prefixes):
    """Generate .kissconfig content from codebase p95 stats."""
    from .engine.discovery import find_source_files
    from .engine.metrics import compute_file_metrics
    from .engine.parser import parse_files
    ...
    lines = ["# Generated by lemon mimic", "[thresholds]"]
    for key in _STATS_METRICS:
        vals = sorted(values.get(key, []))
        if vals:
            lines.append(f"{key} = {max(percentile(vals, 95), 1)}")
    return "\n".join(lines) + "\n"
...
def viz(out, path, lang, ignore_prefixes, zoom):
    ...
    write_viz(graph, out_path, zoom=zoom)
...
def show_tests(path, lang, untested, ignore_prefixes):
    ...
    for d in sorted(analysis.definitions, key=lambda x: (str(x.file), x.line)):
        ...
        cov_key = (d.file, d.name)
        covering = analysis.coverage_map.get(cov_key, [])
        if key in unreferenced_set:
            if untested:
                print(f"UNTESTED:{d.file}:{d.line}:{d.name}")
        elif covering:
            test_names = ", ".join(t[1] for t in covering[:3])
            suffix = f" +{len(covering) - 3} more" if len(covering) > 3 else ""
            print(f"TESTED:{d.file}:{d.name} <- [{test_names}{suffix}]")
        else:
            print(f"TESTED:{d.file}:{d.name}")
